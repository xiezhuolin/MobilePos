...
import cn.acewill.mobile.pos.model.report.BodyRequest;
import cn.acewill.mobile.pos.model.report.DishReport;
import cn.acewill.mobile.pos.printer.Printer;
import cn.acewill.mobile.pos.service.retrofit.RetrofitTerminalService;
import cn.acewill.mobile.pos.service.retrofit.response.LKLResponse;
import cn.acewill.mobile.pos.service.retrofit.response.OtherFileResponse;
import cn.acewill.mobile.pos.service.retrofit.response.PosResponse;
import cn.acewill.mobile.pos.service.retrofit.response.ScreenResponse;
import cn.acewill.mobile.pos.service.retrofit.response.ValidationResponse;
import cn.acewill.mobile.pos.service.retrofit.response.WeiFuTongResponse;
...
public class StoreBusinessService {
	...
	public static StoreBusinessService getInstance() throws PosServiceException {
		if (storeBusinessService == null) {
			RetrofitTerminalService internalService = RetrofitFactory
					.buildService(RetrofitTerminalService.class);
	...
	public void startWorkShift(WorkShift workShift, ResultCallback<WorkShift> resultCallback) {
		...
		Observable ov = internalService
				.startWorkShift(posInfo.getAppId(), posInfo.getStoreId(), workShift)
				.map(new Func1<PosResponse<WorkShift>, WorkShift>() {
					@Override
					public WorkShift call(PosResponse<WorkShift> response) {
						...
						PosServiceException exception = new PosServiceException(ErrorCode.NETWORK_ERROR, response
								.getErrmsg());
						throw Exceptions.propagate(exception);
					}
				}).subscribeOn(Schedulers.io());

		ov.observeOn(AndroidSchedulers.mainThread())
				.subscribe(new ResultSubscriber<WorkShift>(resultCallback));
	}

	public void endWorkShift(Long workShiftId, WorkShift workShift, ResultCallback<Integer> resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService
				.endWorkShift(posInfo.getAppId(), posInfo.getStoreId(), workShiftId, workShift)
				.map(new Func1<PosResponse, Integer>() {
					@Override
					public Integer call(PosResponse response) {
						...
						PosServiceException exception = new PosServiceException(ErrorCode.SERVER_NOT_CONFIGURED, response
								.getErrmsg());
						throw Exceptions.propagate(exception);
					}
				}).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
				.subscribe(new ResultSubscriber<Integer>(resultCallback));
	}
	...
	public void getOpenWorkShift(String userName, String terminalId, ResultCallback<WorkShift> resultCallback) {
		...
		PosInfo posInfo = PosInfo.getInstance();
		internalService
				.getOpenWorkShift(posInfo.getAppId(), posInfo.getStoreId(), userName, terminalId)
				.map(new Func1<PosResponse<WorkShift>, WorkShift>() {
					@Override
					public WorkShift call(PosResponse<WorkShift> response) {
						if (response.getResult() == 0) {
	...
	public void addPrinter(Printer printer, ResultCallback<Printer> resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService
				.addPrinter(posInfo.getAppId(), posInfo.getBrandId(), posInfo.getStoreId(), printer)
				.map(new Func1<PosResponse<Printer>, Printer>() {
					@Override
				...
				.subscribe(new ResultSubscriber<Printer>(resultCallback));
	}

	public void listPrinters(ResultCallback<List<Printer>> resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService.listPrinters(posInfo.getAppId(), posInfo.getBrandId(), posInfo.getStoreId())
				...
				.subscribe(new ResultSubscriber<List<Printer>>(resultCallback));
	}
	...
	public void getWorkShiftDefinition(ResultCallback<List<Definition>> resultCallback) {
		...
		PosInfo posInfo = PosInfo.getInstance();
		internalService
				.workShiftDefinition(posInfo.getAppId(), posInfo.getBrandId(), posInfo.getStoreId())
				.map(new Func1<PosResponse<List<Definition>>, List<Definition>>() {
					@Override
				...
				.subscribe(new ResultSubscriber<List<Definition>>(resultCallback));
	}

	public void deletePrinter(Integer printerId, ResultCallback<Integer> resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService.deletePrinter(posInfo.getAppId(), posInfo.getBrandId(), posInfo
				.getStoreId(), printerId.toString())
				.map(new Func1<PosResponse<Integer>, Integer>() {
					@Override
	...
	public void updatePrinter(Integer printerId, Printer printer, ResultCallback<Printer> resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService.updatePrinter(posInfo.getAppId(), posInfo.getBrandId(), posInfo
				.getStoreId(), printerId, printer).map(new Func1<PosResponse<Printer>, Printer>() {
			@Override
			public Printer call(PosResponse<Printer> response) {
				if (response.getResult() == 0) {
					return response.getContent();
	...
	public void listReceipts(ResultCallback<List<Receipt>> resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService.listReceipts(posInfo.getAppId(), posInfo.getBrandId(), posInfo.getStoreId())
				.map(new Func1<PosResponse<List<Receipt>>, List<Receipt>>() {
					@Override
				...
				.subscribe(new ResultSubscriber<List<Receipt>>(resultCallback));
	}
	...
	public void getStoreBusinessInformation(ResultCallback<StoreBusinessInformation> resultCallback) {
		...
		Observable ov = internalService
				.getStoreBusinessInformation(posInfo.getAppId(), posInfo.getBrandId(), posInfo
						.getStoreId())
				.map(new Func1<PosResponse<StoreBusinessInformation>, StoreBusinessInformation>() {
					@Override
		...
		ov.observeOn(Schedulers.io())
				.subscribe(new ResultSubscriber(new ResultCallback<StoreBusinessInformation>() {
					@Override
		...
		ov.observeOn(AndroidSchedulers.mainThread())
				.subscribe(new ResultSubscriber<StoreBusinessInformation>(resultCallback));
	}
		...
		internalService.getOtherFile(posInfo.getAppId(), posInfo.getBrandId(), posInfo.getStoreId())
				.map(new Func1<OtherFileResponse, List<OtherFile>>() {
					...
					public List<OtherFile> call(OtherFileResponse otherFileResponse) {
						...
						PosServiceException exception = new PosServiceException(ErrorCode.NETWORK_ERROR, otherFileResponse
								.getErrmsg());
						throw Exceptions.propagate(exception);
					}
				})
				.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
				.subscribe(new ResultSubscriber<List<OtherFile>>(resultCallback));
	}
	...
	public void getReportData(String workshiftId, ResultCallback<List<OrderItemReportData>> resultCallback) {
		...
		PosInfo posInfo = PosInfo.getInstance();
		internalService.getReportData(posInfo.getAppId(), posInfo.getBrandId(), posInfo
				.getStoreId(), workshiftId)
				.map(new Func1<PosResponse<List<OrderItemReportData>>, List<OrderItemReportData>>() {
					...
					public List<OrderItemReportData> call(PosResponse<List<OrderItemReportData>> listPosResponse) {
						...
						String errmsg = TextUtils
								.isEmpty(listPosResponse.getErrmsg()) ? "获取报表数据失败" : listPosResponse
								.getErrmsg();
				...
				.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
				.subscribe(new ResultSubscriber<List<OrderItemReportData>>(resultCallback));
	}
	...
	public void getDishCounts(ResultCallback<List<DishCount>> resultCallback) {
		...
		Observable ov = internalService
				.getDishCounts(posInfo.getAppId(), posInfo.getBrandId(), posInfo.getStoreId())
				.map(new Func1<PosResponse<List<DishCount>>, List<DishCount>>() {
					@Override
		...
		ov.observeOn(Schedulers.io())
				.subscribe(new ResultSubscriber(new ResultCallback<List<DishCount>>() {
					@Override
		...
		ov.observeOn(AndroidSchedulers.mainThread())
				.subscribe(new ResultSubscriber<List<DishCount>>(resultCallback));
	}
		...
		internalService.getReportDish("total", "DESC", bodyRequest)
				.map(new Func1<PosResponse<List<DishReport>>, List<DishReport>>() {
					...
					public List<DishReport> call(PosResponse<List<DishReport>> listPosResponse) {
						...
						String errmsg = TextUtils
								.isEmpty(listPosResponse.getErrmsg()) ? "获取菜品数据失败" : listPosResponse
								.getErrmsg();
						PosServiceException exception = new PosServiceException(ErrorCode.NETWORK_ERROR, errmsg);
						throw Exceptions.propagate(exception);
					}
				...
				.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
				.subscribe(new ResultSubscriber<List<DishReport>>(resultCallback));
	}
	...
	public void getTerminalInfo(Context context, String macAddress, ResultCallback<TerminalInfo> resultCallback) {
		...
		internalService.getTerminalInfo(macAddress)
				.map(new Func1<PosResponse<TerminalInfo>, TerminalInfo>() {
					@Override
				...
				.subscribe(new ResultSubscriber<TerminalInfo>(resultCallback));
	}
	...
	public void unbindStore(String userName, String password, String terminalMac, ResultCallback<Integer> resultCallback) {
		internalService.unbindStore(userName, password, terminalMac)
				.map(new Func1<PosResponse, Integer>() {
					@Override
					public Integer call(PosResponse response) {
						if (response.getResult() == 0) {
	...
	public void checkUserAuthority(String username, String password, ResultCallback<Boolean> resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService.checkUserAuthority(username, password, posInfo.getAppId(), posInfo
				.getBrandId(), posInfo.getStoreId(), PowerController.REFUND_DISH)
				...
				.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
				.subscribe(new ResultSubscriber<Boolean>(resultCallback));
	}
	...
	public void bindStore(String terminalMac, ResultCallback<TerminalInfo> resultCallback) {
		...
		Observable ov = internalService.bindStore(terminalMac, Constant.MOBILE_POS)
				.map(new Func1<PosResponse<TerminalInfo>, TerminalInfo>() {
					@Override
					public TerminalInfo call(PosResponse<TerminalInfo> response) {
						if (response.getResult() == 0) {
	...
	public void creatPaymaxCharge(String amount, String subject, String body, String order_no, String channel, String client_ip, String user_id, ResultCallback<LKLResponse> resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService.creatPaymaxCharge(posInfo.getAppId(), posInfo
				.getStoreId(), amount, subject, body, order_no, channel, client_ip, user_id)
				...
				.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
				.subscribe(new ResultSubscriber<LKLResponse>(resultCallback));
	}
	...
	public void getWayWeiFuTong(String auth_code, String body, Integer total_fee, String out_trade_no, ResultCallback<WeiFuTongResponse> resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService.gateway(posInfo.getAppId(), posInfo
				.getStoreId(), auth_code, "192.168.1.168", body, total_fee, out_trade_no)
				...
				.subscribe(new ResultSubscriber<WeiFuTongResponse>(resultCallback));
	}

	public void queryWeiFuTong(String out_trade_no, String transaction_id, ResultCallback<WeiFuTongResponse> resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService.queryWeiFuTong(posInfo.getAppId(), posInfo
				.getStoreId(), out_trade_no, transaction_id)
				.map(new Func1<PosResponse<WeiFuTongResponse>, WeiFuTongResponse>() {
					@Override
					public WeiFuTongResponse call(PosResponse<WeiFuTongResponse> response) {
						ToolsUtils.writeUserOperationRecords(ToolsUtils.getPrinterSth(response));
	...
	public void getStoreConfiguration(ResultCallback<StoreConfiguration> resultCallback) {
		...
		Observable ov = internalService
				.getStoreConfiguration(posInfo.getAppId(), posInfo.getBrandId(), posInfo
						.getStoreId())
				.map(new Func1<PosResponse<StoreConfiguration>, StoreConfiguration>() {
					@Override
		...
		//        ;
		ov.observeOn(AndroidSchedulers.mainThread())
				.subscribe(new ResultSubscriber<StoreConfiguration>(resultCallback));
	...
	public void getWorkShiftHistory(final String startTime, final String endTime, final ResultCallback<List<WorkShift>> resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService.getWorkShiftHistory(posInfo.getAppId(), posInfo.getBrandId(), posInfo
				.getStoreId(), startTime, endTime)
				.map(new Func1<PosResponse<List<WorkShift>>, List<WorkShift>>() {
					@Override
				...
				.subscribe(new ResultSubscriber<List<WorkShift>>(resultCallback));
	}
	...
	public void getScreenConfiguration(ResultCallback<ScreenResponse> resultCallback) {
		...
		Observable ov = internalService
				.getScreenConfiguration(posInfo.getAppId(), posInfo.getBrandId(), posInfo
						.getStoreId())
		...
		ov.observeOn(Schedulers.io())
				.subscribe(new ResultSubscriber(new ResultCallback<ScreenResponse>() {
					@Override
		...
		ov.observeOn(AndroidSchedulers.mainThread())
				.subscribe(new ResultSubscriber<ScreenResponse>(resultCallback));
	}
	...
	public void validationSetout(String code, ResultCallback<ValidationResponse> resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService.validationSetout(posInfo.getAppId(), posInfo.getBrandId(), posInfo
				.getStoreId(), code)
				...
				.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
				.subscribe(new ResultSubscriber<ValidationResponse>(resultCallback));
	}
	...
	public void executeCode(String code, Long orderId, ResultCallback<ValidationResponse> resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService
				.executeCode(posInfo.getAppId(), posInfo.getBrandId(), posInfo.getStoreId(), posInfo
						.getRealname(), orderId, code)
				.map(new Func1<PosResponse<ValidationResponse>, ValidationResponse>() {
					@Override
					public ValidationResponse call(PosResponse<ValidationResponse> response) {
						ToolsUtils.writeUserOperationRecords(ToolsUtils.getPrinterSth(response));
