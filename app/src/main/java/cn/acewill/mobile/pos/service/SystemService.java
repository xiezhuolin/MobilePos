...
import cn.acewill.mobile.pos.factory.CookieInterceptor;
...
import cn.acewill.mobile.pos.model.StandByCash;
...
import cn.acewill.mobile.pos.model.user.Staff;
...
import cn.acewill.mobile.pos.printer.PrinterVendors;
import cn.acewill.mobile.pos.service.retrofit.RetrofitSystemService;
...
import cn.acewill.mobile.pos.service.retrofit.response.TerminalLoginResponse;
import cn.acewill.mobile.pos.service.retrofit.response.UserResponse;
...
public class SystemService {
	...
	public static SystemService getInstance() throws PosServiceException {
		if (systemService == null) {
			RetrofitSystemService internalService = RetrofitFactory
					.buildService(RetrofitSystemService.class);
	...
	public SystemVariablesDao getSystemVariablesDao() {
		return systemVariablesDao;
	}
				...
				.map(new Func1<Response<TerminalLoginResponse>, Integer>() {
					...
					public Integer call(Response<TerminalLoginResponse> response) {
						...
						if (posResponse != null && posResponse.getResult() == 0) {
							...
							if (!TextUtils.isEmpty(posResponse.getBindUUID()) && !TextUtils
									.isEmpty(store.getBindUUID())) {
						...
						PosServiceException exception = new PosServiceException(ErrorCode.TERMINAL_LOGIN_FAIL, posResponse
								.getErrmsg());
						throw Exceptions.propagate(exception);
					}
	...
	public void login(String username, final String password, final ResultCallback resultCallback) {
		...
		Observable ov = internalService
				.login(posInfo.getAppId(), posInfo.getBrandId(), posInfo.getStoreId(), posInfo
						.getTerminalMac(), username, password)
				.map(new Func1<Response<UserResponse>, User>() {
					@Override
					public User call(Response<UserResponse> response) {
						...
						if (userResponse.getResult() == 0) {
							...
							user.setName(user.getUserRet().getUsername());
							cachedDao.saveUser(user);
							return user;

						}
						PosServiceException exception = new PosServiceException(ErrorCode.INVALID_USERNAME_PASSWORD, userResponse
								.getErrmsg());
						throw Exceptions.propagate(exception);
					}
				}).subscribeOn(Schedulers.io());
...
//		}));
		...
		ov.observeOn(AndroidSchedulers.mainThread())
				.subscribe(new ResultSubscriber<User>(resultCallback));
	}
	...
	public void getTerminalVersions(ResultCallback<TerminalVersion> resultCallback) {
		internalService.getTerminalVersions(Constant.CHECK_TERMINAL_ID)
				.map(new Func1<PosResponse<TerminalVersion>, TerminalVersion>() {
					@Override
				...
				.subscribe(new ResultSubscriber<TerminalVersion>(resultCallback));
	}
	...
	public void upLoadLogFile(File file, ResultCallback resultCallback) {
		...
		PosInfo posInfo = PosInfo.getInstance();
		internalService
				.upLoadLog(posInfo.getAppId(), posInfo.getBrandId(), posInfo.getStoreId(), posInfo
						.getTerminalId(), description, body).map(new Func1<PosResponse, Integer>() {
			@Override
	...
	public void getLogoPath(ResultCallback resultCallback) {
		...
		internalService
				.getLogoResource(posInfo.getAppId(), posInfo.getBrandId(), posInfo.getStoreId())
				.map(new Func1<OtherFileResponse, String>() {
					@Override
					public String call(OtherFileResponse orderPosResponse) {
						...
						if (orderPosResponse.isSuccessful()) {
							...
							exception = new PosServiceException(ErrorCode.UNKNOWN_ERROR, orderPosResponse
									.getErrmsg());
						}
						throw Exceptions.propagate(exception);
					}
				}).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
				.subscribe(new ResultSubscriber<String>(resultCallback));
	}
	...
	public void workShiftReport(Integer workShiftReport, String endWorkAmount, ResultCallback<WorkShiftNewReport> resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService.workShiftReport(posInfo.getAppId(), posInfo.getBrandId(), posInfo
				.getStoreId(), workShiftReport, endWorkAmount)
				.map(new Func1<PosResponse<WorkShiftNewReport>, WorkShiftNewReport>() {
					@Override
				...
				.subscribe(new ResultSubscriber<WorkShiftNewReport>(resultCallback));
	}
	...
	public void dailyReport(ResultCallback<WorkShiftReport> resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService.dailyReport(posInfo.getAppId(), posInfo.getBrandId(), posInfo.getStoreId())
				.map(new Func1<PosResponse<WorkShiftReport>, WorkShiftReport>() {
					@Override
				...
				.subscribe(new ResultSubscriber<WorkShiftReport>(resultCallback));
	}
	...
	public void getStoreMarket(final ResultCallback<List<Market>> resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService
				.getStoreMarket(posInfo.getAppId(), posInfo.getBrandId(), posInfo.getStoreId())
				.map(new Func1<PosResponse<List<Market>>, List<Market>>() {
					...
					public List<Market> call(PosResponse<List<Market>> listPosResponse) {
						...
						PosServiceException exception = new PosServiceException(ErrorCode.UNKNOWN_ERROR, listPosResponse
								.getErrmsg());
						throw Exceptions.propagate(exception);
					}
				})
				.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
				.subscribe(new ResultSubscriber<List<Market>>(resultCallback));
	}
	...
	public void getPrinterList(final ResultCallback<List<Printer>> resultCallback) {
		...
		Observable ov = internalService
				.getPrinterList(posInfo.getAppId(), posInfo.getBrandId(), posInfo.getStoreId())
	...
	public void getKDSList(final ResultCallback<List<KDS>> resultCallback) {
		...
		Observable ov = internalService
				.getKDSList(posInfo.getAppId(), posInfo.getBrandId(), posInfo.getStoreId())
				...
				.subscribeOn(Schedulers.io());
		ov.observeOn(AndroidSchedulers.mainThread())
				.subscribe(new ResultSubscriber<List<KDS>>(resultCallback));
	...
	public void getKitchenStalls(final ResultCallback<List<KitchenStall>> resultCallback) {
		...
		Observable ov = internalService
				.getKichenStalls(posInfo.getAppId(), posInfo.getBrandId(), posInfo.getStoreId())
				...
				.subscribeOn(Schedulers.io());
		ov.observeOn(AndroidSchedulers.mainThread())
				.subscribe(new ResultSubscriber<List<KitchenStall>>(resultCallback));
	}
	...
	public void getPrinterVendors(final ResultCallback<List<PrinterVendors>> resultCallback) {
		internalService.getPrinterVendors()
				...
				.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
				.subscribe(new ResultSubscriber<List<PrinterVendors>>(resultCallback));
	}
	...
	public void addPrinter(String vendor, String ip, String description, String linkType, String outputType, String width, String labelHeight, ResultCallback resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService.addPrinter(posInfo.getAppId(), posInfo.getBrandId(), posInfo
				.getStoreId(), vendor, ip, description, linkType, outputType, width, labelHeight)
	...
	public void addKds(String ip, String kdsName, ResultCallback resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService
				.addKds(posInfo.getAppId(), posInfo.getBrandId(), posInfo.getStoreId(), ip, kdsName)
				.map(new Func1<PosResponse, Integer>() {
					@Override
					public Integer call(PosResponse response) {
						if (response.getResult() == 0) {
	...
	public void deletePrinter(Printer printer, ResultCallback resultCallback) {
		...
		internalService.deletePrinter(posInfo.getAppId(), posInfo.getBrandId(), posInfo
				.getStoreId(), printer.getId() + "").map(new Func1<PosResponse, Integer>() {
			@Override
			public Integer call(PosResponse response) {
				if (response.getResult() == 0) {
					return 0;
	...
	public void deleteKds(KDS kds, ResultCallback resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService
				.deleteKds(posInfo.getAppId(), posInfo.getBrandId(), posInfo.getStoreId(), kds
						.getId() + "").map(new Func1<PosResponse, Integer>() {
			@Override
			public Integer call(PosResponse response) {
				if (response.getResult() == 0) {
					return 0;
	...
	public void getStaff(final ResultCallback<List<Staff>> resultCallback) {
		...
		internalService.getUsers(posInfo.getAppId(), posInfo.getBrandId(), posInfo.getStoreId())
				...
				.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
				.subscribe(new ResultSubscriber<List<Staff>>(resultCallback));
	}
	...
	public void addUser(String username, Integer discount, int quartersid, String realname, String jobnumber, ResultCallback resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService.addUser(posInfo.getAppId(), posInfo.getBrandId(), posInfo
				.getStoreId(), username, discount, quartersid, realname, jobnumber)
				.map(new Func1<PosResponse, Integer>() {
					@Override
					public Integer call(PosResponse response) {
						if (response.getResult() == 0) {
	...
	public void modifyStaff(String username, int status, Integer discount, int quartersid, String realname, String jobnumber, ResultCallback resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService.addUser(posInfo.getAppId(), posInfo.getBrandId(), posInfo
				.getStoreId(), username, status, discount, quartersid, realname, jobnumber)
				.map(new Func1<PosResponse, Integer>() {
					@Override
					public Integer call(PosResponse response) {
						if (response.getResult() == 0) {
	...
	public void getStandByCash(final ResultCallback<List<StandByCash>> resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService
				.getStandByCash(posInfo.getAppId(), posInfo.getBrandId(), posInfo.getStoreId())
				...
				.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
				.subscribe(new ResultSubscriber<List<StandByCash>>(resultCallback));
	}
	...
	public void createReceiveHistoryRecord(int reasonid, String reasonName, int outputType, String amount, String createTime, ResultCallback resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService.createReceiveHistoryRecord(posInfo.getAppId(), posInfo.getBrandId(), posInfo
				.getStoreId(), reasonid, reasonName, outputType, amount, posInfo
				.getRealname(), posInfo.getTerminalName(), createTime)
				.map(new Func1<PosResponse, Integer>() {
					@Override
					public Integer call(PosResponse response) {
						if (response.getResult() == 0) {
	...
	public void getAllTemplates(ResultCallback<List<PrinterTemplates>> resultCallback) {
		...
		Observable ov = internalService
				.getAllTemplates(posInfo.getAppId(), posInfo.getBrandId(), posInfo.getStoreId())
				.map(new Func1<PosResponse<List<PrinterTemplates>>, List<PrinterTemplates>>() {
					@Override
		...
		ov.observeOn(AndroidSchedulers.mainThread())
				.subscribe(new ResultSubscriber<List<PrinterTemplates>>(resultCallback));
	}
	...
	public void getOrderDiscountTypes(ResultCallback<List<Discount>> resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService.getOrderDiscountTypes(posInfo.getAppId(), posInfo.getBrandId(), posInfo
				.getStoreId()).map(new Func1<PosResponse<List<Discount>>, List<Discount>>() {
			@Override
			public List<Discount> call(PosResponse<List<Discount>> response) {
				...
				.subscribe(new ResultSubscriber<List<Discount>>(resultCallback));
	}
	...
	public void openCashboxHistory(ResultCallback resultCallback) {
		PosInfo posInfo = PosInfo.getInstance();
		internalService.openCashboxHistory(posInfo.getAppId(), posInfo.getBrandId(), posInfo
				.getStoreId(), posInfo.getRealname(), System.currentTimeMillis(), posInfo
				.getTerminalId(), posInfo.getTerminalName(), posInfo.getWorkShiftName())
				.map(new Func1<PosResponse, Integer>() {
					@Override
					public Integer call(PosResponse response) {
						if (response.getResult() == 0) {
