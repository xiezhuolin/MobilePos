				...
				if (code == 0) {
					systemService.login(userName, pwd, new ResultCallback<User>() {
						...
						public void onResult(User user) {
							...
							systemService.getSystemVariablesDao()
									.save(new SystemVariables("server.url", PosInfo
											.getInstance().getServerUrl()));
							systemService.getSystemVariablesDao()
									.save(new SystemVariables("terminal_name", PosInfo.getInstance()
											.getTerminalName()));
							systemService.getSystemVariablesDao()
									.save(new SystemVariables("terminal_id", PosInfo.getInstance()
											.getTerminalId()));
							systemService.getSystemVariablesDao()
									.save(new SystemVariables("terminal_mac", PosInfo.getInstance()
											.getTerminalMac()));
